class ClubFile < ActiveRecord::Base
  ...
  scope :on, ->(date) { order(:date_in).where(['(date_in <= ? AND date_out >= ?) OR (date_in <= ? AND date_out IS NULL)',date,date,date]) }
  ...
  def validate_in_after_last_out
    errors.add(:date_in, :should_be_after_last_out) if !player.last_date_out.blank? && date_in <= player.last_date_out
  end
